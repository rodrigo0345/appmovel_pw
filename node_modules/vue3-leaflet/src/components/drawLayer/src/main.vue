<template>
    <div class="l-draw-layer">
        <div class="map-draw-wrap" :class="[mode,customClass]" :style="positionClass">
...
<style scoped>
    ...
    /* 水平方向 */
    .map-draw-wrap.horizontal .map-draw{
        width: auto;
        display: inline-flex;
    }
    .map-draw-wrap.horizontal .map-draw-item{
        width: 34px;
        border-bottom: none;
        border-right: 1px solid rgba(0, 0, 0, 0.3);
    }
    .map-draw-wrap.horizontal .map-draw-item:last-child{
        border-right: none;
    }
