...
__export(src_exports, {
  ...
  initAppSeparateWindow: () => initAppSeparateWindow,
  initDevToolsSeparateWindow: () => initDevToolsSeparateWindow,
  initDevToolsSeparateWindowBridge: () => initDevToolsSeparateWindowBridge,
...
var toggleHighPerfMode = defineDevToolsAction("devtools:toggle-high-perf-mode", (_, payload) => {
  (0, import_devtools_kit.toggleHighPerfMode)(payload);
});
...
var devtoolsActionEvents = import_devtools_shared.target[BRIDGE_DEVTOOLS_ACTION_EVENTS_KEY] = /* @__PURE__ */ new Map();
var devtoolsListenerEvents = import_devtools_shared.target[BRIDGE_DEVTOOLS_LISTENER_EVENTS_KEY] = /* @__PURE__ */ new Map();
...
function setBridgeTarget(t) {
  import_devtools_shared.target[BRIDGE_TARET_KEY] = t;
}
function getBridgeTarget() {
  return import_devtools_shared.target[BRIDGE_TARET_KEY];
}
...
function setupAppBridge(bridge) {
  ...
  bridge.on(BRIDGE_DEVTOOLS_LISTENER_KEY, async (payload) => {
    ...
    if (listener) {
      listener(import_devtools_kit2.devtools, (res) => {
...
function defineDevToolsAction(name, action) {
  return async (...args) => {
    ...
    if (target4 === "devtools") {
      ...
      const uniqueEventKey = (0, import_non_secure.nanoid)();
...
function defineDevToolsListener(name, fn, options = {}) {
  return (listener) => {
    ...
    if (target4 === "devtools") {
      ...
      const parserFn = parser === "devtools" ? import_devtools_kit3.parse : JSON.parse;
      const bridge = getBridgeInstance();
      const uniqueEventKey = (0, import_non_secure.nanoid)();
...
// src/injection.ts
var import_devtools_kit5 = require("@vue/devtools-kit");
  ...
  onnConnect() {
    return new Promise((resolve) => {
      ...
      this.socket.on("syn-ack" /* SYN_ACK */, () => {
        ...
        import_devtools_kit4.devtools.state.clientConnected = true;
        resolve();
      });
    });
  }
...
function prepareInjection(bridge) {
  ...
  new HandShakeClient(bridge).onnConnect().then(() => {
    bridge.on("devtools:client-ready", () => {
      (0, import_devtools_kit5.onDevToolsConnected)(() => {
        import_devtools_kit5.devtools.state.connected = true;
      });
    });
  });
}
...
// ../../node_modules/.pnpm/vite-hot-client@0.2.3_vite@5.2.8/node_modules/vite-hot-client/dist/index.mjs
...
function callViteServerAction(name) {
  return async (...args) => {
    ...
    const uniqueEventKey = (0, import_non_secure2.nanoid)();
    return new Promise((resolve) => {
      const cb = (e) => {
        var _a;
        if (viteClient.off)
